...
public sealed class PromptBuilder(PromptTemplates templates)
{
    ...
    /// <summary>Builds the compose prompt by injecting inventory item blocks, optional target context, and optional user ethos.</summary>
        ...
        string? userEthos = null)
    {
        ...
        return templates.ComposeTemplate
            ...
            .Replace("{UserEthos}", FormatUserEthos(userEthos))
            .Replace("{InventoryItems}", itemsSection);
    }

    /// <summary>
    /// Formats the user's personal ethos as a prompt section.
    /// Returns an empty string when <paramref name="userEthos"/> is null or whitespace.
    /// </summary>
    public static string FormatUserEthos(string? userEthos)
    {
        if (string.IsNullOrWhiteSpace(userEthos)) return string.Empty;
        ...
        int totalMaxChars = 20000)
    {
        ...
        foreach ((string filename, string path) in priorPaths.Reverse())
        {
            ...
            {
                // File unreadable — fall back to a path reference
